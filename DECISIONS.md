...
- **Do not:** Try to fix a Plotly legend clipping/positioning bug by tweaking `Plotly.Plots.resize()`/`Plotly.react()` timing, container CSS, or `entrywidth`. If the legend renders web-font text and clips, replace it with an HTML/CSS grid legend instead.

### 2026-07-02 — Verify browser/timing bugs against real data before building a synthetic repro
- **Why:** Spent significant effort trying to cleanly reproduce the legend font-swap race with a synthetic `@font-face` + `local()` setup — it never reliably recreated the actual timing window, and repeated attempts timed out. Tunneling to the real production Postgres via `fly proxy` and rendering the actual chart-building code against real data with real network-loaded fonts reproduced (and later confirmed-fixed) the bug cleanly on the first try.
- **Scope:** Any bug suspected to be timing-dependent (font loads, resource loads, race conditions) in this app, where a live data source is reachable (Fly Postgres via `fly proxy`, or the deployed app directly).
- **Do not:** Default to building an increasingly elaborate synthetic browser reproduction for a timing-sensitive bug before trying the real data source. Try real data first; only reach for synthetic repros if real data genuinely isn't reachable.
